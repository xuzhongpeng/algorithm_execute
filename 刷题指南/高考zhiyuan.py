    ...
    for items in res.json()['data']['item']:
      if('spname' in items): #存在spname才执行，如西南科技大学
        if (re.match( r'计算机', items['spname'], re.M|re.I) and 'min_section' in  items and  int(items['min_section']) > 20000 and int(items['min_section']) < 30000):
          id=items['school_id']
          s = requests.get(url='https://static-data.eol.cn/www/2.0/school/{id}/info.json')
          name = ''
          school = s.json()
          if 'data' in school:
            name = school['data']['name']
          print(items['school_id'],'\t',name,'\t',str(times)+"年"+str(major)+"录取最高分",items['max'],'\t',"平均分：",items['average'],'\t',"最低分：",items['min'],'\t',"最低位次:",items['min_section'],'\t',"录取批次:",items['local_batch_name'],'\n')
